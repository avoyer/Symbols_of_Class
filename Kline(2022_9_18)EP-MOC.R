...
write.table(tidy_EP_chunk_n, "kline(2022_5_17)EP-MOC_chunkn.csv", row.names = FALSE)
...
write.table(tidy_EP_all, "kline(2022_5_17)EP-MOC_all.csv", row.names = FALSE)
...
write.table(tidy_EP_DT, "kline(2022_5_17)EP-MOC_chunks.csv", row.names = FALSE)
...
write.csv(tfidf_EP_cos_sim_df, "kline(2022_5_17)EP-MOC_table1.csv")
...
# sensitivity analysis, cosign similarity
# w2vec context
...
#Closest to
close_affluence <- w2vModel%>% closest_to('wealth', 100)
close_employment <- w2vModel%>% closest_to('employment', 100)
close_status <- w2vModel%>% closest_to('status', 100)
close_cultivation <- w2vModel%>% closest_to('cultivation', 100)
close_education <- w2vModel%>% closest_to('education', 100) 
close_morality <- w2vModel%>% closest_to('morality', 100) 


#rename
close_affluence   <- transmute(close_affluence,   word_affluence    = word)
close_employment  <- transmute(close_employment,  word_employment   = word)
close_status      <- transmute(close_status,      word_status       = word)
close_cultivation <- transmute(close_cultivation, word_cultivation  = word)
close_education   <- transmute(close_education,   word_education    = word)
close_morality    <- transmute(close_morality,    word_morality     = word)
...
close_affluence   <- add_row_numbers(close_affluence,   name = "rank", zero_based = FALSE)
close_employment  <- add_row_numbers(close_employment,  name = "rank", zero_based = FALSE)
close_status      <- add_row_numbers(close_status,      name = "rank", zero_based = FALSE)
close_cultivation <- add_row_numbers(close_cultivation, name = "rank", zero_based = FALSE)
close_education   <- add_row_numbers(close_education,   name = "rank", zero_based = FALSE)
close_morality    <- add_row_numbers(close_morality,    name = "rank", zero_based = FALSE)


# merge

close <- inner_join(close_affluence, close_employment)
close <- inner_join(close          , close_status)
close <- inner_join(close          , close_cultivation)
close <- inner_join(close          , close_education)
close <- inner_join(close          , close_morality)



# write to table
write.table(close, "close.csv", row.names = FALSE)
