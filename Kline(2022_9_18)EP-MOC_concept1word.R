...
write.table(tidy_EP_chunk_n, "kline(2022_5_17)EP-MOC_chunkn-1word.csv", row.names = FALSE)
...
tidy_EP_employment <- tidy_EP_full %>% 
  cast_dtm(term = word, 
           ...
           weighting = tm::weightTf)  %>%
  removeSparseTerms(.999) %>%
  CMDist(cw = "employment", wv=fastTM)
...
tidy_EP_cultivation <- tidy_EP_full %>% 
  cast_dtm(term = word, 
           ...
           weighting = tm::weightTf)  %>%
  removeSparseTerms(.999) %>%
  CMDist(cw = "cultivation", wv=fastTM)
...
write.table(tidy_EP_all, "kline(2022_5_17)EP-MOC_all-1word.csv", row.names = FALSE)
...
write.table(tidy_EP_DT, "kline(2022_5_17)EP-MOC_chunks-1word.csv", row.names = FALSE)
...
write.csv(tfidf_EP_cos_sim_df, "kline(2022_5_17)EP-MOC_table1-1word.csv")
